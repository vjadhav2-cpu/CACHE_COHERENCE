// C++ Testbench for Verilator - Direct DUT simulation
// Drives spi_dma_xbar_memory_system directly without Verilog testbench

#include <verilated.h>
#include <verilated_vcd_c.h>
#include "Vspi_dma_xbar_memory_system.h"
#include <iostream>
#include <iomanip>
#include <vector>
...
class Testbench {
public:
    Vspi_dma_xbar_memory_system *dut;
    VerilatedVcdC *tfp;
    ...
    Testbench() {
        dut = new Vspi_dma_xbar_memory_system;
        tfp = nullptr;
        write_count = 0;
        read_count = 0;
    ...
    void tick() {
        // Handle SPI read requests before clock edge
        ...
        // Capture SPI write events after positive edge
    ...
    void handle_spi_read_request() {
        ...
        if (dut->spi_rd_req_valid_tb && !spi_rd_req_prev) {
            ...
            // Update prev IMMEDIATELY to prevent recursive detection when tick() is called
            spi_rd_req_prev = true;
